...
def load_data(input_fn, tokenizer):
    ...
    with open(input_fn) as fin:
        for line in fin:
            s = json.loads(line)
            if (
                "result" not in s.keys()
                or "text" not in s["result"].keys()
                or len(s["result"]["text"].strip()) == 0
            ):
                continue
...
def prepare_ngrams(s, n, tokenizer, max_n=0):
    ...
    return {
        "target_ngrams": g_ngrams_list,
        "doc_list": doc_list,
        "doc_ngrams": doc_ngrams_list,
        "target_tokens": gtokens,
        "doc_token_id_list": doc_token_id_list,
    }
...
def base_generate(
    ...
    if forced_decoding:
        eos = torch.tensor(
            ...
            dtype=gen_texts_ids.dtype,
            device=gen_texts_ids.device,
        )
    ...
    while True:
        with torch.no_grad():
            ...
            if forced_decoding:
                output_ids = gen_texts_ids[:, step : step + step_length].to(
                    output_ids.device
                )
...
def llma_generate(
    ...
    step = 0
    while True:
        prefix_n, g_ngrams, doc_ngrams = match_prefix(
            g_ngrams_list, doc_ngrams_list, step
        )
        if prefix_n > 0:
            copy_mode = True
            ...
            copied_ids = torch.tensor(
                [copied_ids], dtype=prepend_ids.dtype, device=prepend_ids.device
            )
        ...
        with torch.no_grad():
            ...
            if copy_mode:
                ...
                for pos in range(1, len(oids)):
                    if (
                        oids[pos - 1] == iids[pos]
                        and oids[pos - 1] != tokenizer.eos_token_id
                    ):
                ...
                past_key_values = make_past_key_values(
                    past_key_values, step_length, accepted_step_length
                )
...
def run_time_test(
    ...
    for s in s_list:
        ...
        if "result" in s.keys() and "text" in s["result"]:
            gen_texts_ids = tokenizer(
                s["result"]["text"], return_tensors="pt"
            ).input_ids[:, 1:]
        else:
            gen_texts_ids = None
